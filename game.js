var game = {
  ...
  onload: function() {
    ...
    for (a = 0; a < PREFS.MAP_FILES.value.length; a++) game.resources = SID ? game.resources.concat([{
      name: PREFS.MAP_FILES.value[a].split("/").pop().slice(0,
        -4),
      type: "image",
      ...
      type: "image",
      src: "data/img/map/" + PREFS.MAP_FILES.value[a]
    }]);
    for (a = 0; a < PREFS.LEVELS_FILES.value.length; a++) game.resources = SID ? game.resources.concat([{
      ...
      name: "level" + (a + 1),
      type: "tmx",
      src: "data/map/" + PREFS.LEVELS_FILES.value[a]
    }]);
    for (a = 0; a < PREFS.SPRITES_FILES.value.length; a++) game.resources =
      SID ? game.resources.concat([{
        ...
        name: PREFS.SPRITES_FILES.value[a].split("/").pop().slice(0, -4),
        type: "image",
        src: "data/img/sprite/" + PREFS.SPRITES_FILES.value[a]
      }]);
  ...
  loaded: function() {
    ...
    var a = (navigator.userLanguage ? navigator.userLanguage : navigator.language ?
      navigator.language : "english").substring(0, 2); - 1 == PREFS.GAME_LANGUAGES.value.indexOf(a) && (a = PREFS.GAME_LANGUAGES.value[0]);
    me.loader.load({
      name: "locale",
      type: "json",
      src: "data/locale/" + a + ".json"
    }, function() {
...
function getAheadTile(a) {
  ...
  var e = null;
  try {
    e = game.data.Path_Layer.getTile(a.pos.x + b, a.pos.y + d)
  } catch (f) {
    console.info("reached map border")
  }
  return null != e ? (property = e.tileset.getTileProperties(e.tileId), "undefined" != typeof property ? parseInt(property.direction,
    2) : 0) : 0
}
  ...
  update: function(a) {
    ...
    switch (this.state) {
      case "ready":
        ...
        break;
      case "wait":
        if (0 != game.gamepad.axes[1] || 0 != game.gamepad.axes[0]) me.input.triggerKeyEvent(me.input.KEY.LEFT,
          !0), me.input.triggerKeyEvent(me.input.KEY.LEFT, !1);
        break;
      case "done":
...
var swipe = me.Renderable.extend({
    ...
    onMoveEvent: function(a) {
      ...
      if (10 < Math.abs(b) || 10 < Math.abs(d)) me.input.triggerKeyEvent(me.input.KEY.RIGHT,
        !1), me.input.triggerKeyEvent(me.input.KEY.LEFT, !1), me.input.triggerKeyEvent(me.input.KEY.UP, !1), me.input.triggerKeyEvent(me.input.KEY.DOWN, !1), Math.abs(b) > Math.abs(d) ? 0 < b ? me.input.triggerKeyEvent(me.input.KEY.LEFT, !0) : me.input.triggerKeyEvent(me.input.KEY.RIGHT, !0) : 0 < d ? me.input.triggerKeyEvent(me.input.KEY.UP, !0) : me.input.triggerKeyEvent(me.input.KEY.DOWN, !0), this.startPos = a
    },
    destroy: function() {
      me.input.releasePointerEvent("pointerdown", this, this.onStartEvent.bind(this));
  ...
  tap = me.Renderable.extend({
    ...
    onEndEvent: function(a) {
      me.input.triggerKeyEvent(me.input.KEY.RIGHT, !1);
      me.input.triggerKeyEvent(me.input.KEY.LEFT,
        !1);
      me.input.triggerKeyEvent(me.input.KEY.UP, !1);
      me.input.triggerKeyEvent(me.input.KEY.DOWN, !1)
    },
...
var ProgressBar = me.Renderable.extend({
  ...
  TextLogo = me.Renderable.extend({
    ...
    draw: function(a) {
      ...
      navigator.isCocoonJS || a.drawImage(this.image,
        (this.width - this.image.width) / 2, this.height / 2 - this.image.height - this.imageH)
    }
  }),
...
var helpImage = me.Entity.extend({
    init: function(a, b) {
      ...
      this.renderable.addAnimation("5",
        [4])
    },
    update: function(a) {
      switch (helpPage) {
